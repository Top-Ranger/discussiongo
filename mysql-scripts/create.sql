...
CREATE TABLE discussiongo.user (name VARCHAR(600) NOT NULL, salt VARCHAR(600), encodedpasswort VARCHAR(600), admin BOOLEAN, comment VARCHAR(600) DEFAULT '', invitedby VARCHAR(600) DEFAULT '', invitationdirect BOOL DEFAULT 0, lastseen BIGINT UNSIGNED DEFAULT 0, PRIMARY KEY(name));
CREATE TABLE discussiongo.topic (id BIGINT UNSIGNED AUTO_INCREMENT, name VARCHAR(600), creator VARCHAR(600), created BIGINT UNSIGNED, lastmodified BIGINT UNSIGNED, closed BOOL DEFAULT 0, pinned BOOL DEFAULT 0, PRIMARY KEY(id));
CREATE INDEX idx_topic_lastmodified_desc ON discussiongo.topic (lastmodified DESC);
CREATE TABLE discussiongo.post (id BIGINT UNSIGNED AUTO_INCREMENT, content VARCHAR(600), poster VARCHAR(600), time BIGINT UNSIGNED, topic BIGINT UNSIGNED, FOREIGN KEY(topic) REFERENCES topic(id) ON UPDATE CASCADE ON DELETE CASCADE, PRIMARY KEY(id));
